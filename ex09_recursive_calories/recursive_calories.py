...
def x_sum_recursion(nums, x) -> int:
    ...
    if x < 0:
        nums.reverse()
    return x_sum_recursion(nums[abs(x):], abs(x)) + nums[abs(x) - 1]
...
def lets_count_calories(salad: float, chocolate_pieces: int, fridge_visits: int) -> int:
    ...
    :param fridge_visits: number of fridge visits.
    ...
    return lets_count_calories(salad - eaten_salad, chocolate_pieces - eaten_chocolate, fridge_visits - 1) \
        + current_calories
